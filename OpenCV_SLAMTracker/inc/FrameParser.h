...
public:
	cv::Mat matOrignImage[2], matOutputImage[2];
	cv::Mat matDescriptor[2];
	cv::Mat matCameraParam;
...
public:
	...
	void MatchFeaturePoints(/*img0,1 point0,1,& matchMat*/);
	...
	//静态方法
	static int DetectExtractFeatures(int nFeatures, cv::Mat& matImage, FeatureState& fState, int idxImg);
